...
(function(factory) {
	if (typeof define === 'function' && define.amd) {
		define([ '$eajQ', 'moment' ], factory);
	}
	else {
		factory($eajQ, moment);
	}
})(function($eajQ, moment) {
...
function generateShortTimeFormat(options, langData) {
	return langData.longDateFormat('LT')
		.replace(':mm', '(:mm)')
		.replace(/(\Wmm)$eajQ/, '($1)') // like above, but for foreign langs
...
var fc = $eajQ.fullCalendar = { version: "2.2.2" };
var fcViews = fc.views = {};


$eajQ.fn.fullCalendar = function(options) {
	var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
	var res = this; // what this function will return (this $eajQ object by default)

	this.each(function(i, _element) { // loop each DOM element involved
		var element = $eajQ(_element);
		var calendar = element.data('fullCalendar'); // get the existing calendar object (if any)
		var singleRes; // the returned value of this single method call

		// a method call
		if (typeof options === 'string') {
			if (calendar && $eajQ.isFunction(calendar[options])) {
...
// Better than `$eajQ.extend(true, ...)` because arrays are not traversed/copied.
//
// called like:
...
function mergeOptions(target) {
	...
	function mergeIntoTarget(name, value) {
		if ($eajQ.isPlainObject(value) && $eajQ.isPlainObject(target[name]) && !isForcedAtomicOption(name)) {
	...
	for (var i=1; i<arguments.length; i++) {
		$eajQ.each(arguments[i], mergeIntoTarget);
	}
...
// Initialize $eajQ UI Datepicker translations while using some of the translations
...
fc.datepickerLang = function(langCode, datepickerLangCode, options) {
	...
	// is $eajQ UI Datepicker is on the page?
	if ($eajQ.datepicker) {
		...
		$eajQ.datepicker.regional[datepickerLangCode] =
			$eajQ.datepicker.regional[langCode] = // alias
				options;

		// Alias 'en' to the default language data. Do this every time.
		$eajQ.datepicker.regional.en = $eajQ.datepicker.regional[''];

		// Set as Datepicker's global defaults.
		$eajQ.datepicker.setDefaults(options);
	}
};
...
function Calendar(element, instanceOptions) {
	...
	function initialRender() {
		...
		content = $eajQ("<div class='fc-view-container'/>").prependTo(element);
		...
		if (options.handleWindowResize) {
			...
			$eajQ(window).resize(windowResizeProxy);
		}
	}
	...
	function destroy() {
		...
		$eajQ(window).unbind('resize', windowResizeProxy);
	}
	...
	function renderView(delta, viewName) {
		...
		if (!currentView && viewName) {
			...
			currentView.el =  $eajQ("<div class='fc-view fc-" + viewName + "-view' />").appendTo(content);
...
function Header(calendar, options) {
	...
	function renderSection(position) {
		var sectionEl = $eajQ('<div class="fc-' + position + '"/>');
		var buttonStr = options.header[position];

		if (buttonStr) {
			$eajQ.each(buttonStr.split(' '), function(i) {
				var groupChildren = $eajQ();
				var isOnlyButtons = true;
				var groupEl;

				$eajQ.each(this.split(','), function(j, buttonName) {
					var buttonClick;
					var themeIcon;
					var normalIcon;
					var defaultText;
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($eajQ('<h2>&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
						...
						if (buttonClick) {
							...
							button = $eajQ( // type="button" so that it doesn't submit a form
...
function EventManager(options) { // assumed to be a calendar
	...
	function fetchEventSource(source, fetchID) {
		_fetchEventSource(source, function(eventInputs) {
			var isArraySource = $eajQ.isArray(source.events);
			var i, eventInput;
			var abstractEvent;
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				// retrieve any outbound GET/POST $eajQ.ajax data from the options
				var customData;
				if ($eajQ.isFunction(source.data)) {
				...
				var data = $eajQ.extend({}, customData || {});
				...
				pushLoading();
				$eajQ.ajax($eajQ.extend({}, ajaxDefaults, source, {
					data: data,
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		...
		if ($eajQ.isFunction(sourceInput) || $eajQ.isArray(sourceInput)) {
			...
			source = $eajQ.extend({}, sourceInput); // shallow copy
		}
		...
		if (source) {
			...
			if ($eajQ.isArray(source.events)) {
				source.origArray = source.events; // for removeEventSource
				source.events = $eajQ.map(source.events, function(eventInput) {
	...
	function removeEventSource(source) {
		sources = $eajQ.grep(sources, function(src) {
			return !isSourcesEqual(src, source);
		});
		// remove all client events from that source
		cache = $eajQ.grep(cache, function(e) {
	...
	function removeEvents(filter) {
		...
		else if (!$eajQ.isFunction(filter)) { // an event ID
			eventID = filter + '';
		...
		cache = $eajQ.grep(cache, filter, true); // inverse=true

		// Remove events from array sources.
		// This works because they have been converted to official Event Objects up front.
		// (and as a result, event._id has been calculated).
		for (i=0; i<sources.length; i++) {
			if ($eajQ.isArray(sources[i].events)) {
				sources[i].events = $eajQ.grep(sources[i].events, filter, true);
			}
		}
	...
	function clientEvents(filter) {
		if ($eajQ.isFunction(filter)) {
			return $eajQ.grep(cache, filter);
		}
		else if (filter != null) { // not null, not undefined. an event ID
			filter += '';
			return $eajQ.grep(cache, function(e) {
				return e._id == filter;
			});
		}
			...
			if (abstractEvent._recurring) {
				...
				while (date.isBefore(_rangeEnd)) {
					...
					if (!dowHash || dowHash[date.day()]) { // if everyday, or this particular day-of-week
						...
						event = $eajQ.extend({}, abstractEvent); // make a copy of the original
						assignDatesToEvent(
							start, end,
	...
	function mutateEvents(events, clearEnd, forceAllDay, dateDelta, durationDelta) {
		...
		$eajQ.each(events, function(i, event) {
			var oldAllDay = event._allDay;
			var oldStart = event._start;
			var oldEnd = event._end;
	...
	function getBusinessHoursEvents() {
		...
		if (optionVal) {
			if (typeof optionVal === 'object') {
				...
				eventInput = $eajQ.extend({}, defaultVal, optionVal);
			}
			else {
...
function disableCursor() {
	$eajQ('body').addClass('fc-not-allowed');
}


// Returns the mouse cursor to its original look
function enableCursor() {
	$eajQ('body').removeClass('fc-not-allowed');
}
...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	els.each(function(i, el) {
		...
		var naturalOffset = $eajQ(el).outerHeight(true);

		if (naturalOffset < minOffset) {
			flexEls.push(el);
			flexOffsets.push(naturalOffset);
			flexHeights.push($eajQ(el).height());
		}
		else {
	...
	$eajQ(flexEls).each(function(i, el) {
...
// Given `els`, a $eajQ set of <td> cells, find the cell with the largest natural width and set the widths of all the
...
function matchCellWidths(els) {
	...
	els.find('> *').each(function(i, innerEl) {
		var innerWidth = $eajQ(innerEl).outerWidth();
...
// borrowed from https://github.com/$eajQ/$eajQ-ui/blob/1.11.0/ui/core.js#L51
function getScrollParent(el) {
	var position = el.css('position'),
		scrollParent = el.parents().filter(function() {
			var parent = $eajQ(this);
			return (/(auto|scroll)/).test(
				parent.css('overflow') + parent.css('overflow-y') + parent.css('overflow-x')
			);
		}).eq(0);

	return position === 'fixed' || !scrollParent.length ? $eajQ(el[0].ownerDocument || document) : scrollParent;
}
...
var oldMomentProto = $eajQ.extend({}, newMomentProto); // copy of original moment methods
...
Options:
	- className (string)
	- content (HTML string or $eajQ element set)
	- parentEl
	- top
	- left
...
Popover.prototype = {
	...
	render: function() {
		...
		this.el = $eajQ('<div class="fc-popover"/>')
		...
		if (options.autoHide) {
			$eajQ(document).on('mousedown', this.documentMousedownProxy = $eajQ.proxy(this, 'documentMousedown'));
		}
	},
	...
	documentMousedown: function(ev) {
		...
		if (this.el && !$eajQ(ev.target).closest(this.el).length) {
			this.hide();
		}
	},
	...
	destroy: function() {
		...
		$eajQ(document).off('mousedown', this.documentMousedownProxy);
	},
	...
	startListening: function(ev) {
		...
		if (!this.isListening) {
			...
			if (ev && this.options.scroll) {
				scrollParent = getScrollParent($eajQ(ev.target));
				if (!scrollParent.is(window) && !scrollParent.is(document)) {
					this.scrollEl = scrollParent;

					// scope to `this`, and use `debounce` to make sure rapid calls don't happen
					this.scrollHandlerProxy = debounce($eajQ.proxy(this, 'scrollHandler'), 100);
			...
			$eajQ(document)
				.on('mousemove', this.mousemoveProxy = $eajQ.proxy(this, 'mousemove'))
				.on('mouseup', this.mouseupProxy = $eajQ.proxy(this, 'mouseup'))
	...
	setScrollVel: function(topVel, leftVel) {
		...
		if ((this.scrollTopVel || this.scrollLeftVel) && !this.scrollIntervalId) {
			this.scrollIntervalId = setInterval(
				$eajQ.proxy(this, 'scrollIntervalFunc'), // scope to `this`
...
function MouseFollower(sourceEl, options) {
	...
	this.parentEl = options.parentEl ? $eajQ(options.parentEl) : sourceEl.parent(); // default to sourceEl's parent
}
...
MouseFollower.prototype = {
	...
	start: function(ev) {
		if (!this.isFollowing) {
			...
			$eajQ(document).on('mousemove', this.mousemoveProxy = $eajQ.proxy(this, 'mousemove'));
		}
	},
	...
	stop: function(shouldRevert, callback) {
		...
		if (this.isFollowing && !this.isAnimating) { // disallow more than one stop animation at a time
			...
			$eajQ(document).off('mousemove', this.mousemoveProxy);
...
RowRenderer.prototype = {
	...
	// `cells` can be an HTML string of <td>'s or a $eajQ <tr> element
	// `row` is an optional row number.
...
$eajQ.extend(Grid.prototype, {

	el: null, // the containing element
	coordMap: null, // a GridCoordMap that converts pixel values to datetimes
	cellDuration: null, // a cell's duration. subclasses must assign this ASAP
	elsByFill: null, // a hash of $eajQ element sets used for rendering each fill. Keyed by fill name.
	...
	// DOM and $eajQ will be smart enough to garbage collect the handlers.
	bindHandlers: function() {
		var _this = this;

		this.el.on('mousedown', function(ev) {
			if (
				!$eajQ(ev.target).is('.fc-event-container *, .fc-more') && // not an an event element, or "more.." link
				!$eajQ(ev.target).closest('.fc-popover').length // not on a popover (like the "more.." events one)
			) {
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$eajQ(html).each(function(i, node) {
				var seg = segs[i];
				var el = $eajQ(node);
				...
				if (el) { // custom filters did not cancel the render
					el = $eajQ(el); // allow custom filter to return raw DOM node
...
$eajQ.extend(Grid.prototype, {
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			$eajQ(html).each(function(i, node) {
				var seg = segs[i];
				var el = view.resolveEventEl(seg.event, $eajQ(node));
		...
		$eajQ.each(
			{
				...
				mousedown: function(seg, ev) {
					if ($eajQ(ev.target).is('.fc-resizer') && view.isEventResizable(seg.event)) {
			...
			function(name, func) {
				...
				_this.el.on(name, '.fc-event-container > *', function(ev) {
					var seg = $eajQ(this).data('fc-seg'); // grab segment data. put there by View::renderEvents
	...
	eventsToRanges: function(events) {
		...
		$eajQ.each(eventsById, function(id, eventGroup) {
...
$eajQ.extend(DayGrid.prototype, {
	...
	render: function(isRigid) {
		...
		this.dayEls.each(function(i, node) {
			...
			view.trigger('dayRender', null, date, $eajQ(node));
		});
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			var rowEl = $eajQ(rowNode); // the .fc-row
			var skeletonEl = $eajQ('<div class="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
		...
		this.helperEls = $eajQ(helperNodes); // array -> $eajQ set
	},
	...
	renderFill: function(type, segs) {
		...
		this.elsByFill[type] = $eajQ(nodes);
...
$eajQ.extend(DayGrid.prototype, {
	...
	renderBgSegs: function(segs) {
		...
		var allDaySegs = $eajQ.grep(segs, function(seg) {
	...
	renderFgSegs: function(segs) {
		...
		this.rowEls.each(function(i, rowNode) {
			$eajQ(rowNode).find('.fc-content-skeleton > table').append(
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $eajQ('<td class="fc-event-container"/>').append(seg.el);
...
$eajQ.extend(DayGrid.prototype, {
	...
	limitRow: function(row, levelLimit) {
		...
		var cellMatrix; // a matrix (by level, then column) of all <td> $eajQ elements in the row
		...
		function emptyCellsUntil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
				...
				if (segsBelow.length) {
					...
					moreWrap = $eajQ('<div/>').append(moreLink);
					td.append(moreWrap);
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $eajQ('<td class="fc-more-cell"/>').attr('rowspan', rowspan);
					...
					td.addClass('fc-limited').after($eajQ(segMoreNodes)); // hide original <td> and inject replacements
					limitedNodes.push(td[0]);
				}
			}

			emptyCellsUntil(view.colCnt); // finish off the level
			rowStruct.moreEls = $eajQ(moreNodes); // for easy undoing later
			rowStruct.limitedEls = $eajQ(limitedNodes); // for easy undoing later
		}
	},
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		var events = $eajQ.map(segs, function(seg) {
			return seg.event;
		});
...
$eajQ.extend(TimeGrid.prototype, {
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.helperEl = $eajQ('<div class="fc-helper-skeleton"/>')
			.append(tableEl)
				.appendTo(this.el);
	},
	...
	renderFill: function(type, segs, className) {
		...
		if (segs.length) {
			...
			for (col = 0; col < segCols.length; col++) {
				colSegs = segCols[col];
				tdEl = $eajQ('<td/>').appendTo(trEl);

				if (colSegs.length) {
					containerEl = $eajQ('<div class="fc-' + className + '-container"/>').appendTo(tdEl);
...
$eajQ.extend(TimeGrid.prototype, {
	...
	renderFgSegs: function(segs) {
		...
		this.el.append(
			this.eventSkeletonEl = $eajQ('<div class="fc-content-skeleton"/>')
	...
	renderSegTable: function(segs) {
		var tableEl = $eajQ('<table><tr/></table>');
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			containerEl = $eajQ('<div class="fc-event-container"/>');
			...
			trEl.append($eajQ('<td/>').append(containerEl));
		}
...
View.prototype = {
	...
	init: function() {
		...
		this.documentMousedownProxy = $eajQ.proxy(this, 'documentMousedown');
		this.documentDragStartProxy = $eajQ.proxy(this, 'documentDragStart');
	},
	...
	documentDragStart: function(ev, ui) {
		...
		if (this.opt('droppable')) { // only listen if this setting is on
			...
			if ($eajQ.isFunction(accept) ? accept.call(el[0], el) : el.is(accept)) {
				...
				dragListener = new DragListener(this.coordMap, {
					cellOver: function(cell, cellDate) {
						...
						if (eventProps) {
							$eajQ.extend(eventProps, { start: eventStart, end: eventEnd });
						}
				...
				$eajQ(document).one('dragstop', function(ev, ui) {
					var renderedEvents;
	...
	documentMousedown: function(ev) {
		...
		if (this.isSelected && this.opt('unselectAuto') && isPrimaryMouseButton(ev)) {
			...
			if (!ignore || !$eajQ(ev.target).closest(ignore).length) {
				this.unselect(ev);
			}
		}
	}
...
function View(calendar) {
	...
	function opt(name) {
		var v = options[name];
		if ($eajQ.isPlainObject(v) && !isForcedAtomicOption(name)) {
	...
	(function() {
		...
		for (var dayIndex=0, cellIndex=0; dayIndex<7; dayIndex++) {
			...
			isHiddenDayHash[dayIndex] = $eajQ.inArray(dayIndex, hiddenDays) != -1;
...
// Given a $eajQ element that might represent a dragged FullCalendar event, returns an intermediate data structure
...
function getDraggedElMeta(el) {
	...
	if (eventProps) {
		if (typeof eventProps === 'object') {
			eventProps = $eajQ.extend({}, eventProps); // make a copy
		}
...
$eajQ.extend(BasicView.prototype, {
...
$eajQ.extend(MonthView.prototype, {
...
$eajQ.extend(BasicWeekView.prototype, {
...
$eajQ.extend(BasicDayView.prototype, {
...
$eajQ.extend(AgendaView.prototype, {
	...
	render: function(colCnt) {
		...
		this.bottomRuleEl = $eajQ('<hr class="' + this.widgetHeaderClass + '"/>')
...
$eajQ.extend(AgendaWeekView.prototype, {
...
$eajQ.extend(AgendaDayView.prototype, {
