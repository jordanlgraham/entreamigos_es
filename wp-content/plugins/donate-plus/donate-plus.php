...
if( !class_exists('DonatePlus') ):
	class DonatePlus{
		function DonatePlus() { //constructor
			//ACTIONS
				#Add Settings Panel
		...
		function icon_css(){
			...
			#toplevel_page_DonatePlus div.wp-menu-image {
			...
			#toplevel_page_DonatePlus:hover div.wp-menu-image, #toplevel_page_DonatePlus.current div.wp-menu-image, #toplevel_page_DonatePlus.wp-has-current-submenu div.wp-menu-image {
		...
		function DefaultSettings () {
			...
			if( !get_option('DonatePlus') ): #Set Defaults if no values exist
				add_option( 'DonatePlus', $default );
				...
				endforeach;
				if( $new )
					update_option( 'DonatePlus', $dplus );
			endif;
		}
		...
		function Settings(){
			...
			?>
             <div class="wrap">
            	<h2><?php _e('Donate Plus Settings', 'dplus')?></h2>
...
<strong><?php _e('Replacement Codes', 'dplus');?></strong> <small><?php _e('Email Message Only', 'dplus');?></small><br /><?php _e('<code>{donor}</code> = Donor Name<br \><code>{amount}</code> = Donation Amount<br \><code>{donorwall}</code> = Donor Wall URL','dplus');?></td>
               ...
               <p><code><?php echo str_replace(ABSPATH, trailingslashit(get_option('siteurl')), dirname(__FILE__)).'/paypal.php';?></code><br /><?php _e('This is your IPN Notification URL.  If you have issues with your site receiving your PayPal payments, be sure to manually set this URL in your PayPal Profile IPN settings.  You can also view your ', 'dplus');?> <a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=_display-ipns-history"><?php _e('IPN History on PayPal','dplus');?></a></p>
                <h2><?php _e('Uninstall Donate Plus Tables and Options', 'dplus'); ?></h2>
                <form method="post" action="">
                	<?php if( function_exists( 'wp_nonce_field' )) wp_nonce_field( 'dplus-delete'); ?>  
                    <p><?php _e('<strong>WARNING:</strong> Uninstalling the Donate Plus tables and option settings will remove all donation data related to this plugin.  This data will not be recoverable.','dplus');?></p>
                    <p class="submitdelete" style="text-align:center"><input name="Submit" value="<?php _e('Uninstall Donate Plus','dplus');?>" type="submit" /><input name="action" value="dplus_delete" type="hidden" /></p>
...
function DonatePlusForm(){
	global $donateplus;
	echo $donateplus->DonatePage();
}
function DonatePlusWall(){
	global $donateplus;
	echo $donateplus->DonorWall();
}
function DonatePlusTotal(){
	global $donateplus;
