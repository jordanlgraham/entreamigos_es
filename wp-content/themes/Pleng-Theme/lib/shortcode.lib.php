...
function nivoslide_func($atts, $content) {
	...
	$rand_id = mt_rand();
	$return_html = '<div id="'.$rand_id.'" class="nivoslide" style="width:'.$width.'px;height:'.$height.'px; visibility: hidden">';
	...
	if(!empty($image_arr) && is_array($image_arr))
	{
		foreach($image_arr as $image)
		{
			...
			if(!empty($image))
			{
				$return_html.= '<img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.strip_tags($image).'&amp;h='.$height.'&amp;w='.$width.'&amp;zc=1" alt=""/>'. PHP_EOL;
			}
		}
	}
	
	$return_html.= '</div><br class="clear"/><br class="clear"/>';
...
function flickr_func($atts, $content) {
	...
	if(!empty($flickr_id))
	{
		...
		{
		    $return_html.= '<li>';
		    $return_html.= '<a href="'.$photo['url'].'" title="'.$photo['title'].'"><img src="'.$photo['thumb_url'].'" alt="" class="frame" /></a>';$return_html.= '</li>';
		}
