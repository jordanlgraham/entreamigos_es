		...
		?>
				...
				<div id="nivo_slider" class="nivoSlider">
							...
							<a href="<?php echo $hyperlink_url;?>">
								<img src="<?php bloginfo( 'stylesheet_directory' ); ?>/timthumb.php?src=<?php echo htmlentities($image_url[0]); ?>&amp;h=360&amp;w=960&amp;zc=1" alt="" <?php if(!empty($caption_style)) { echo 'title="'.$caption_style.'"'; } ?>/>
							</a>
...
$j(window).load(function() {
	$j('#nivo_slider').nivoSlider({ pauseTime: parseInt($j('#slider_timer').val() * 1000), pauseOnHover: false, effect: 'sliceDown', controlNav: true, captionOpacity: 1, directionNavHide: <?php echo $pp_homepage_slider_nav; ?>, controlNavThumbs:false, controlNavThumbsFromRel:false, afterLoad: function(){ 
